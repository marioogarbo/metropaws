...
| `seed.py` | **Reference data for a fresh database**, one named function per subject in `SEEDERS` order: service types, plans, plan services, app settings, admin user, PawPoints rewards, sample clinics (only with `SEED_CLINIC_PASSWORD`), FAQs. Every step is idempotent — it inserts only what is missing — so re-running tops a database up after a new default is added, and never overwrites an admin's edit. Nothing runs on import; `python seed.py`, minding `APP_ENV`. Not part of the app lifecycle |
